...
DUK_LOCAL duk_bool_t duk__get_own_propdesc_raw(duk_hthread *thr, duk_hobject *obj, duk_hstring *key, duk_uint32_t arr_idx, duk_propdesc *out_desc, duk_small_uint_t flags) {
	 ...
	 *  virtual property matches.  FIXME
	 */

	/* FIXME: fast path skip for no exotics */
...
DUK_INTERNAL duk_bool_t duk_hobject_getprop(duk_hthread *thr, duk_tval *tv_obj, duk_tval *tv_key) {
	...
	case DUK_TAG_OBJECT: {
		...
		/* FIXME: array .length fast path (important in e.g. loops)? */
...
DUK_LOCAL duk_uint32_t duk__to_new_array_length_checked(duk_hthread *thr) {
	...
	/* FIXME: fastint; avoid value stack! */
...
duk_bool_t duk__handle_put_array_length_smaller(duk_hthread *thr,
	...
	a = (duk_harray *) obj;
	DUK_ASSERT_HARRAY_VALID(a);
	DUK_UNREF(a);  /* FIXME: remove if not needed */
...
DUK_LOCAL duk_bool_t duk__handle_put_array_length(duk_hthread *thr, duk_hobject *obj) {
	...
	/* FIXME: making array .length non-writable, needs handling...
	 * length starts with attribute 'W', and isn't configurable,
	 * so the only allowed transition is to lose the writability.
	 */
	...
	if (duk__get_own_propdesc_raw(thr, obj, key, arr_idx, &desc, 0 /*flags*/)) {  /* don't push value */
		if (desc.e_idx >= 0) {
			...
			if (key == DUK_HTHREAD_STRING_LENGTH(thr) && DUK_HOBJECT_HAS_EXOTIC_ARRAY(obj)) {
				/* FIXME: avoid this case entirely by fixing call sites? */
#if defined(DUK_USE_DEBUG)
				duk_uint32_t prev_len;
				prev_len = ((duk_harray *) obj)->length;
#endif
				((duk_harray *) obj)->length = duk_to_uint32(ctx, -1);  /* FIXME; proper conversion */
...
void duk_hobject_define_property_helper(duk_context *ctx,
	...
	if (key == DUK_HTHREAD_STRING_LENGTH(thr)) {
		...
		/* FIXME: handle array .length here already */
		...
		if (arr_idx >= old_len) {
			...
			if (a->length_nonwritable) {
				/* FIXME: force flag? add duk_def_prop() testcase! */
				goto fail_not_writable_array_length;
			}
 ...
 need_check:
	...
	/* FIXME: for ROM objects too it'd be best if value modify was
	...
	if (curr.e_idx >= 0) {
		...
		if (key == DUK_HTHREAD_STRING_LENGTH(thr) && DUK_HOBJECT_HAS_EXOTIC_ARRAY(obj)) {
			...
			DUK_ASSERT_HARRAY_VALID(a);
			a->length_nonwritable = (new_flags & DUK_PROPDESC_FLAG_WRITABLE ? 0 : 1);
		}
	}

	/* FIXME: safety for virtual case */
	...
	if (has_value) {
		...
		if (curr.e_idx >= 0) {
			...
			/* FIXME: Uint8Array etc virtual writes... how to handle? */
			if (key == DUK_HTHREAD_STRING_LENGTH(thr) && DUK_HOBJECT_HAS_EXOTIC_ARRAY(obj)) {
				duk_harray *a;
				a = (duk_harray *) obj;
 ...
 success_exotics:
	...
	if (DUK_HOBJECT_HAS_EXOTIC_ARRAY(obj)) {
		...
		/* FIXME: need .length special handling in all cases */
